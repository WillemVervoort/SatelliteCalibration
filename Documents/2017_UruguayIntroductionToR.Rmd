...
# Introduction  
This is an introduction to R written for the "How do I use satellite and global reanalysis data for hydrological simulations in SWAT?" workshop in Montevideo between 7 - 11 August 2017. This work is based on earlier "introduction to R" practicals in units of study at the University of Sydney and an introduction to R course taught in Guernavaca, Mexico in 2014. It also builds on many of the introduction to R literature on the internet.  
...
```{r resetwd, echo=F}
setwd("c:/users/rver4657/ownCloud/SouthSouthNetwork/Resources/IntroductionToR")
```
```{r read_csv}
...
x[,3] <- ifelse(x[,2] > 5000, "large", "small")
...
```{r nestedifelse}
x[,4] <- ifelse(x[,2] > 2500,ifelse(x[,2] > 10000, "large", "intermediate"), "small")
tail(x,20)
```
...
![Structure of GR4J](GR4JStructure.png)
